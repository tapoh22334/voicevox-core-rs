...
impl VoicevoxCore {
    ...
    ///
    pub unsafe fn predict_duration(&self, phoneme_vector: &[i64], speaker_id: u32) -> Result<CPointerWrap<f32>, ResultCode> {
        let len = phoneme_vector.len();
        let ptr = phoneme_vector.as_ptr() as *mut i64;
        let mut data_length: usize = 0;
        let mut data_ptr: *mut f32 = std::ptr::null_mut();
        let result_code = voicevox_predict_duration(len,
                                                    ptr,
                                                    speaker_id,
                                                    &mut data_length,
                                                    &mut data_ptr);
    ...
    /// # Arguments
    ///
    /// * `vowel_phoneme_vector` - A mutable slice of `i64` representing vowel phoneme vectors.
    /// * `consonant_phoneme_vector` - A mutable slice of `i64` representing consonant phoneme vectors.
    /// * `start_accent_vector` - A mutable slice of `i64` representing start accent vectors.
    /// * `end_accent_vector` - A mutable slice of `i64` representing end accent vectors.
    /// * `start_accent_phrase_vector` - A mutable slice of `i64` representing start accent phrase vectors.
    /// * `end_accent_phrase_vector` - A mutable slice of `i64` representing end accent phrase vectors.
    ...
    pub unsafe fn predict_intonation(
        ...
        let result_code = voicevox_predict_intonation(
            length,
            vowel_ptr,
            consonant_ptr,
            start_accent_ptr,
        ...
        match result_code {
            0 => {
                let ptr_wrap = CPointerWrap::new(
                    ...
                    |p| voicevox_predict_intonation_data_free(p),
                );
                Ok(ptr_wrap)
            }
            e => Err( unsafe { std::mem::transmute(e) } ),
    ...
    pub unsafe fn decode(&self,
        ...
        let result_code = voicevox_decode(
            phoneme_size,
            f0_size,
            phoneme_ptr,
            f0_ptr,
    ...
    pub fn voicevox_synthesize(audio_query: &str, speaker_id: u32, options: VoicevoxSynthesisOptions) -> Result<Wav, ResultCode> {
        ...
        match result_code {
            0 => {
                let wav = Wav::new(output_wav_ptr, output_wav_length, |p| unsafe{ voicevox_wav_free(p) } );
                Ok(wav)
            }
            e => Err( unsafe { std::mem::transmute(e) } ),
        }
        ...
        match result_code {
            0 => {
                let ptr_wrap = CStrWrap::new(
                    output_ptr,
                    |p| voicevox_audio_query_json_free(p),
                );
                Ok(ptr_wrap)
            }
    ...
    fn _tts(text: &str,
        ...
        match result {
            0 => {
                let wav = Wav::new(out_wav, out_length, |p| unsafe{ voicevox_wav_free(p) } );
                Ok(wav)
            }
